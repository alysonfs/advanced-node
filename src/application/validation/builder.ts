import { Required, RequiredBuffer, RequiredString, Validator, AllowedMimeType, Extension, MaxFileSize } from '@/application/validation'
...
export class ValidationBuilder {
  ...
  image ({ allowed, maxSizeInMb }: { allowed: Extension[], maxSizeInMb: number }): ValidationBuilder {
    if (this.value.mimeType !== undefined) {
      this.validators.push(new AllowedMimeType(allowed, this.value.mimeType))
    }
    if (this.value.buffer !== undefined) {
